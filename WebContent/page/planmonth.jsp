    ...
    <div class="summary_mina">
      <div class="summary_mina_con">
        <div  class="tijiaoadd">
          ...
          <div class="xiangmu_show"><a href="planmonth">当月项目</a></div>
          <div class="xiangmu_show2"><a href="planhistory">历史项目</a></div>
        </div>
        <div class="renwu_info">
         
            <table>
            <tr><td width="80px">序号</td><td width="280px">工作项目</td><td width="180px">责任人</td><td width="180px">计划完成时间</td><td width="180px">操作</td></tr>
            <s:iterator value="#request.page.results" var="item" status="st">
            	<tr><td><s:property value="#st.index+1"/></td><td><s:property value="#item.title"/></td><td><s:property value="#item.completer.name"/></td><td><s:date name="#item.completeDate" format="yyyy-MM-dd"></s:date></td><td><s:if test="#item.status==1"><button onclick="planCon(<s:property value="#item.id"/>);">确认</button></s:if><s:else><button onclick="" disabled="disabled">已确认</button></s:else></td></tr>
            </s:iterator>
            </table>
